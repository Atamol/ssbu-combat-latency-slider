...
初回起動時に`sd:/config/ssbu-combat-latency-slider/config.txt`が生成されます．
...
featureを直接指定した場合 (例: `cargo skyline build --release --features 2f`) は，cargo-skylineに出力名を変えるオプションがないため通常と同じ`libssbu_combat_latency_slider.nro`が出力されます．
